...
const API_KEY = "52da92b75a1d7dc3fab389809e0f4433";
...
async function chargerNetflopTMDB() {
  console.log("Chargement des données depuis TMDB...");
...
async function afficherFilmsPopulaires() {
  ...
  try {
    ...
    //console.log(data);
    const container = document.getElementById("films-populaires");
    container.innerHTML = "";
    ...
    const slider = creerSlider(data.results.slice(0, 15), "film");
...
async function afficherSeriesPopulaires() {
  ...
  try {
    ...
    const data = await response.json();
    //console.log(data);
    const container = document.getElementById("series-populaires");
    container.innerHTML = "";
    ...
    const slider = creerSlider(data.results.slice(0, 15), "serie");
    container.appendChild(slider);
  } catch (error) {
    console.error("Erreur lors de la requête HTTP :", error);
  }
...
async function afficherDocumentaires() {
  const url = `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=99&sort_by=popularity.desc&language=fr-FR&page=1`;
  ...
  try {
    ...
    const data = await response.json();
    //console.log(data);
    const container = document.getElementById("documentaires");
    container.innerHTML = "";
    ...
    const slider = creerSlider(data.results.slice(0, 15), "documentaire");
    container.appendChild(slider);
  } catch (error) {
    console.error("Erreur lors de la requête HTTP :", error);
  }
...
async function afficherAnimes() {
  const url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=16&with_origin_country=JP&sort_by=popularity.desc&language=fr-FR&page=1`;
  ...
  try {
    ...
    const slider = creerSlider(data.results.slice(0, 15), "anime");
    container.appendChild(slider);
  } catch (error) {
    console.error("Erreur lors de la requête HTTP :", error);
  }
...
function creerSlider(items, type) {
    ...
    boutonPrecedent.classList.add("bouton-precedent");
    boutonPrecedent.innerHTML = "◄";
    boutonPrecedent.disabled = true;

    const boutonSuivant = document.createElement("button");
    boutonSuivant.classList.add("bouton-suivant");
    boutonSuivant.innerHTML = "►";

    slider.appendChild(boutonPrecedent);
    slider.appendChild(boutonSuivant);

    const wrapper = document.createElement("div");
    wrapper.classList.add("slider-wrapper");

    items.forEach((item) => {
        const card = creerCarteTMDB(item, type);
